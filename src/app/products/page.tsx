    ...
    <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 w-full">
      ...
      <FadeIn direction="up">
        <div className="flex flex-col md:flex-row justify-between items-end md:items-center mb-8 gap-4">
          <div>
            <h1 className="text-3xl font-bold text-brand-dark">Tất cả sản phẩm</h1>
            <p className="text-brand-text-secondary mt-2">Hiển thị {MOCK_PRODUCTS.length} kết quả</p>
          </div>
          ...
          <FadeIn direction="right" delay={0.1}>
            <div>
              ...
              <ul className="space-y-3">
                {['Tất cả', 'Áo Thun', 'Áo Khoác', 'Quần Jean', 'Quần Short', 'Phụ Kiện'].map((category, i) => (
                  <li key={i}>
                    <button className={`text-sm hover:text-brand-accent transition-colors ${i === 0 ? 'font-bold text-brand-accent' : 'text-gray-600'}`}>
                      {category}
                    </button>
                  </li>
                ))}
